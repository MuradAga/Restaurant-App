document.addEventListener('DOMContentLoaded', () => {
    ...
    function showModal(content) {
        modalMessage.innerHTML = content.replace(/\n/g, '<br>');
    ...
    function updateTexts() { document.title = getTranslation('title'); document.querySelectorAll('[data-key]').forEach(el => { el.textContent = getTranslation(el.dataset.key); }); renderCategoryButtons(); renderMenuItems(); renderCart(); updateActiveOrderDisplays(); }
    ...
    [backToHomeFromMenu, backToHomeFromHesabIste, waiterBackBtn, backToHomeFromConfirmed].forEach(btn => btn.addEventListener('click', () => showScreen('welcome-screen')));
    ...
    menuGrid.addEventListener('click', (e) => {
        ...
        if (action === 'increase') {
            if (cItem) {
                ...
                // DÜZƏLİŞ BURADADIR
                const newItem = { ...menuItems.find(i => i.id === id), quantity: 1 };
                cart.push(newItem);
                cItem = newItem; // Yeni yaradılan elementi cItem-a mənimsədirik
            }
        ...
        const qSpan = document.getElementById(`quantity-${id}`);
        if (qSpan) {
            // cItem-in mövcud olub-olmamasından asılı olaraq yeniləyirik
            const currentCartItem = cart.find(c => c.id === id);
            qSpan.textContent = currentCartItem ? currentCartItem.quantity : 0;
        }
        renderCart();
    });
    ...
    confirmOrderBtn.addEventListener('click', () => {
        if (cart.length === 0) { showModal(getTranslation('selectItems')); return; }
        const currentTotal = parseFloat(totalPriceSpan.textContent);
        if (lastConfirmedOrder) {
            cart.forEach(newItem => {
                const existingItem = lastConfirmedOrder.items.find(item => item.id === newItem.id);
                if (existingItem) {
                    existingItem.quantity += newItem.quantity;
                } else {
                    lastConfirmedOrder.items.push(JSON.parse(JSON.stringify(newItem)));
                }
            });
            lastConfirmedOrder.total = (parseFloat(lastConfirmedOrder.total) + currentTotal).toFixed(2);
        } else {
            lastConfirmedOrder = {
    ...
    const displayOrderDetails = () => {
        if (!lastConfirmedOrder) { showModal("Aktiv sifariş yoxdur."); return; }
        let detailsText = `<b>${getTranslation('orderDetails')}</b>\n--------------------\n`;
        detailsText += `<b>${getTranslation('activeTable')}</b> №${lastConfirmedOrder.tableNumber}\n\n`;
        lastConfirmedOrder.items.forEach(item => { detailsText += `${item.quantity}x ${getTranslation(`item_${item.id}_name}`)} - <b>${(item.quantity * item.price).toFixed(2)} AZN</b>\n`; });
        detailsText += `\n<b>${getTranslation('totalPrice')} ${lastConfirmedOrder.total} AZN</b>`;
    ...
    billOkBtn.addEventListener('click', () => {
        ...
        if (!paymentMethod) { showModal(getTranslation('selectPayment')); return; }
